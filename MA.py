...
#Optimize function
def optimize(chromosome,M):
	#First optimization
	for i in range(M/2):
		if (chromosome[2][i]-chromosome[2][i+M/2])<2:
			x=chromosome[2][i]+2
			if x>5:
				x-=4
			chromosome[2][i+M/2]=x#Changing day of 2nd lecture of a subject.
	#Second optimization
	for j in range(M):
		for k in range(M):
			if chromosome[2][j]==chromosome[2][k] and chromosome[3][j]==chromosome[2][k]:
				if chromosome[1][j]==chromosome[1][k]:
					chromosome[3][k]=random.randint(1,8)#Changing slot of a subject, if there is a  clash.
	...
	for j in range(M):
		for k in range(M):
			if chromosome[2][j]==chromosome[2][k] and chromosome[3][j]==chromosome[2][k]:
				if chromosome[1][j]==chromosome[1][k]:
					x=chromosome[2][k]
					chromosome[2][k]=random.randint(1,8)#Changing slot of a subject, if there is a  clash.
					chromosome[2][k]=x
